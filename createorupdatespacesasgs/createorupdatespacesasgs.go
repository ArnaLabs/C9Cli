package createorupdatespacesasgs
...
import (
	...
	"os/exec"
	"time"
	"github.com/ArnaLabs/C9Cli/createorupdateasgs"
)
					...
					for j := 0; j < SpaceLen; j++ {
						...
						if _, err := guid.Output(); err == nil{
							...
							if InitClusterConfigVals.ClusterDetails.EnableASG == true {
								...
								createorupdateasgs.CreateOrUpdateASGs(Orgs.Org.Name, Orgs.Org.Spaces[j].Name, ASGPath, ostype, ASGName)
							} else {
								fmt.Println("Enable ASGs: ", InitClusterConfigVals.ClusterDetails.EnableASG)
